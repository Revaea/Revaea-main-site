...
const numberOpacityVars = (value: number): gsap.TweenVars => ({
...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  ...
  const buildOpenTimeline = useCallback(() => {
    ...
    if (numberEls.length) gsap.set(numberEls, numberOpacityVars(0));
  ...
  return (
    <>
      <header
        className={`staggered-menu-header pointer-events-none z-50 ${
          isFixed ? 'fixed top-0 left-0 w-full' : 'absolute top-0 left-0 w-full'
        } flex items-center justify-between p-[2em] bg-transparent`}
        aria-label="Main navigation header"
      >
        {displayLogo && (
          <div className="sm-logo flex h-4 items-center select-none pointer-events-auto" aria-label="Logo">
            <Image
              src={logoUrl || '/apple.png'}
              alt="Logo"
              className="sm-logo-img block h-4 w-auto object-contain"
              draggable={false}
              width={16}
              height={16}
            />
        ...
        <button
          ref={toggleBtnRef}
          className={`sm-toggle inline-flex h-4 items-center gap-[0.3rem] bg-transparent border-0 cursor-pointer leading-none overflow-visible pointer-events-auto ${
            toggleClassName ? '' : 'font-medium text-[#e9e9ef]'
          } ${toggleClassName || 'ml-auto relative'}`}
          style={toggleStyle}
      ...
      >
        ...
        >
          {(() => {
            ...
            return arr.map((c, i) => (
              <div
                key={i}
                className={`sm-prelayer absolute top-0 right-0 h-full w-full ${initialHiddenClass}`}
                style={{ background: c }}
              />
            ));
          })()}
        ...
        <aside
          ...
          ref={panelRef}
          className={`staggered-menu-panel pointer-events-auto absolute top-0 right-0 h-full bg-white flex flex-col p-[6em_2em_2em_2em] overflow-y-auto z-10 backdrop-blur-[12px] ${initialHiddenClass}`}
...
.sm-scope .staggered-menu-panel { position: absolute; top: 0; right: 0; width: clamp(260px, 38vw, 480px); height: 100%; background: white; backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px); display: flex; flex-direction: column; padding: 6em 2em 2em 2em; overflow-y: auto; z-index: 10; }
.sm-scope [data-position='left'] .staggered-menu-panel { right: auto; left: 0; }
.sm-scope .sm-prelayers { position: absolute; top: 0; right: 0; bottom: 0; width: clamp(260px, 38vw, 480px); pointer-events: none; z-index: 5; }
